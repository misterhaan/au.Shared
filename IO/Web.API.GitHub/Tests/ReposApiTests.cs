...
namespace au.IO.Web.API.GitHub.Tests {
	...
	public class ReposApiTests {
		...
		public async Task LatestRelease_NoRepo_Throws404() {
			...
			Assert.AreEqual(HttpStatusCode.NotFound, caughtException.StatusCode, $"{nameof(api.LatestRelease)}() is expected to throw an error with a 404 not found status code when the requested repo doesn't exist.  Did GitHub user {_username} create a repo named {_repoFake}?");
		}
		...
		public async Task LatestRelease_NoReleases_Throws404() {
			...
			Assert.AreEqual(HttpStatusCode.NotFound, caughtException.StatusCode, $"{nameof(api.LatestRelease)}() is expected to throw an error with a 404 not found status code when the requested repo doesn't have any releases.  Did GitHub repo {_username}/{_repoNoReleases} have a release?");
		}
